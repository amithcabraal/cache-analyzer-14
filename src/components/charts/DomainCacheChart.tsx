...
export function DomainCacheChart({ data, title, onFilterChange }: DomainCacheChartProps) {
  const prepareData = (): ChartData[] => {
    ...
    data.forEach(request => {
      try {
        ...
        domainData[domain][rank.rank] = (domainData[domain][rank.rank] as number) + bytes;
      } catch {
        // Skip invalid URLs
      }
    });
